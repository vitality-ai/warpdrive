...
TENANT_ID     = "b4d750dee5f9e459ba0a31f92635053e"  # fresh native-prepare tenant, no-slab leg (2026-08-23)
...
RESULTS_ROOT  = Path("/home/nash/cj/warpdrive/warpdrive/docs/benchmarks/logs/scaling_noslab_gcp")
...
ENDPOINTS = [
    # 10 working endpoints (have pgdata)
    ("main",  55432),
    ("ep-2",  55451),
    ("ep-5",  55465),
    ("ep-6",  55466),
    ...
    ("ep-g",  55486),
    ("ep-h",  55487),
    # 6 fresh branch endpoints (branched from main at LSN 1/9447B078)
    ("ep-k",  55490),
    ("ep-l",  55492),
    ("ep-m",  55494),
    ("ep-n",  55496),
...
def run_one(T, dry_run=False):
    ...
    # Artifacts: a pageserver log slice scoped to exactly this run (the log
    # file itself spans every run across every day, which caused a real
    # false-alarm mix-up earlier -- byte-offset-scoping it here avoids that),
    # plus the raw WarpDrive metrics snapshots (startup and final) for
    # standalone inspection without needing to re-query a live server later.
    try:
