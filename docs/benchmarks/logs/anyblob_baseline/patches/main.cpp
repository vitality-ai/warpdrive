...
int main(int argc, char* argv[]) {
    ...
    helpText += "-w endpointHost [required for minio]\n";
    helpText += "-p endpointPort [required for minio]\n";
    ...
    for (auto i = 3; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "-b")) {
            ...
            awsSettings.endpoint = argv[++i];
        } else if (!strcmp(argv[i], "-p")) {
            awsSettings.port = static_cast<uint32_t>(atoi(argv[++i]));
    ...
    if (!strcmp(argv[1], "minio") && bandwithSettings.systems.front() == anyblob::benchmark::Bandwidth::Systems::Uring) {
        string uri = "minio://" + awsSettings.endpoint + ":" + to_string(awsSettings.port) + "/" + awsSettings.bucket + ":" + awsSettings.region + "/";
