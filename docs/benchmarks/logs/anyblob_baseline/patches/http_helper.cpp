...
//---------------------------------------------------------------------------
namespace {
bool ciEquals(string_view a, string_view b) {
    if (a.size() != b.size())
        return false;
...
// Detect the protocol
{
    ...
    for (auto& keyValue : info.response.headers) {
        if (ciEquals(transferEncoding, keyValue.first) && ciEquals(chunkedEncoding, keyValue.second)) {
        ...
        } else if (ciEquals(contentLength, keyValue.first)) {
