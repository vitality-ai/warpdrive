...
pub struct MockMetadataStore {
    data: Arc<Mutex<HashMap<String, HashMap<String, Metadata>>>>,
}
...
impl MockMetadataStore {
    ...
    pub fn object_count(&self, user_id: &str) -> usize {
        let data = self.data.lock().unwrap();
        data.get(user_id).map(|user_data| user_data.len()).unwrap_or(0)
    }
}
...
impl MetadataStorage for MockMetadataStore {
    fn put_metadata(&self, user_id: &str, object_id: &str, metadata: &Metadata) -> Result<(), Error> {
        ...
        if user_data.contains_key(object_id) {
            return Err(actix_web::error::ErrorBadRequest("Key already exists"));
        }
        
        user_data.insert(object_id.to_string(), metadata.clone());
        Ok(())
    }
    ...
    fn get_metadata(&self, user_id: &str, object_id: &str) -> Result<Metadata, Error> {
        ...
        if let Some(user_data) = data.get(user_id) {
            if let Some(metadata) = user_data.get(object_id) {
    ...
    fn delete_metadata(&self, user_id: &str, object_id: &str) -> Result<(), Error> {
        ...
        if let Some(user_data) = data.get_mut(user_id) {
            if user_data.remove(object_id).is_some() {
                return Ok(());
            }
        }
    ...
    fn list_objects(&self, user_id: &str) -> Result<Vec<ObjectId>, Error> {
        ...
        if let Some(user_data) = data.get(user_id) {
            Ok(user_data.keys().cloned().collect())
        } else {
            Ok(Vec::new())
        }
    }
    ...
    fn object_exists(&self, user_id: &str, object_id: &str) -> Result<bool, Error> {
        ...
        if let Some(user_data) = data.get(user_id) {
            Ok(user_data.contains_key(object_id))
        } else {
            Ok(false)
        }
    }
    ...
    fn update_object_id(&self, user_id: &str, old_object_id: &str, new_object_id: &str) -> Result<(), Error> {
        ...
        if let Some(user_data) = data.get_mut(user_id) {
            if let Some(metadata) = user_data.remove(old_object_id) {
                user_data.insert(new_object_id.to_string(), metadata);
                return Ok(());
            }
        }
...
mod tests {
    ...
    fn test_mock_metadata_store_basic_operations() {
        ...
        let result = store.put_metadata(user_id, object_id, &metadata);
    ...
    fn test_mock_metadata_store_error_cases() {
        ...
        let result = store.get_metadata(user_id, object_id);
        assert!(result.is_err());
        
        // Test delete_metadata for nonexistent object
        let result = store.delete_metadata(user_id, object_id);
        assert!(result.is_err());
        
        // Test update_metadata for nonexistent object
        let metadata = Metadata::from_offset_size_list(vec![(100, 200)]);
        let result = store.update_metadata(user_id, object_id, &metadata);
        assert!(result.is_err());
        
        // Test update_object_id for nonexistent object
        let result = store.update_object_id(user_id, object_id, "new_id");
        assert!(result.is_err());
        
        // Test list_objects for nonexistent user
        let objects = store.list_objects("nonexistent_user").unwrap();
