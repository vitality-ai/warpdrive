...
2. Fault Tolerance - Uses Erasure Coding to Optimize data replication - Seeks contribution for design - [Discussion](https://github.com/cia-labs/Storage-service/issues/72)
   - Use case: Vitality Console dataset and model-artifact storage needs resilient replicas so training and inference workloads can recover from node or disk failures.
...
4. Search - Seeks contribution for design. - [Discussion](https://github.com/cia-labs/Storage-service/issues/35)
   - Use case: Vitality Console and AI data pipelines need fast object lookup by metadata/key patterns to discover datasets, checkpoints, and snapshots without manual path traversal.
5. Availability - Seeks contribution for design - [Discussion (to be created)](https://github.com/vitality-ai/warpdrive/issues/new/choose)
   - Use case: production WarpDrive deployments serving training and data-ingestion workloads require high uptime and quick failover so long-running jobs are not interrupted.
