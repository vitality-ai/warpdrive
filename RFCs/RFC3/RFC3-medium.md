...
There are roughly a dozen well-known storage systems that have already solved everything in this article. LFS, Haystack, Bitcask, WiscKey, TitanDB, RocksDB, F2FS, NILFS, Dostoevsky. Segment-based storage, space reclamation via whole-file deletion, write amplification vs space amplification trade-offs, workload-aware threshold tuning - all of it studied, published, implemented, and shipped. The ideas in record management are not new. They have been beaten to death. I had fun anyway but in the middle of all of this i realised what is new in all of this?

This is a write-up of RFC3 from [Warpdrive](https://github.com/vitality-ai/warpdrive), an S3-compatible object storage server I have been building in Rust. The full RFC, all plots, and the optimizer script are in the [repository](https://github.com/vitality-ai/warpdrive/blob/feat/rfc-segment-storage/warpdrive/RFCs/RFC3/RFC3-segment-storage.md).
...
Each block is a 512 MB file on disk. A single number - the global offset stored in SQLite - tells you exactly which file to open and where to seek inside it. No new columns, no mapping table. Reads just do two integer operations (divide and modulo) before opening the file.
...
The orange region in the utilization plot is wasted disk space - bytes that are dead but still occupy storage. Deleting 70% of the data drops live bytes immediately but disk usage stays flat. That gap is space amplification in action. Compaction closes it by copying survivors out of cold segments and deleting those segment files entirely. Without compaction the orange region grows forever.
...
Each curve in the objective surface plot is a different priority weighting. The dots show optimal T for each weighting. SA-heavy and balanced weightings both hit T = 0.5 - the optimizer always pushes toward aggressive compaction because that is the only way to reduce SA. WA-heavy weightings pull T down because background compaction I/O becomes costly. Our proposed T = 0.30 is conservative - we accepted worse SA to avoid compacting segments that might only be temporarily cold.
...
Each point on the Pareto frontier is an operating point - a choice of T giving a specific WA/SA trade-off. The frontier collapses to a single point (T = 0.5, SA = 3.0) for most weightings because the optimizer always hits the cap. The proposed point sits off the frontier with higher SA than optimal - that is the cost of keeping T conservative at 0.30.
...
- High delete rate: segments go cold quickly regardless. T = 0.4-0.5 is reasonable.
...
To minimise SA you maximise T. T = 1 means compact after every single byte dies - constant compaction. The practical cap is T = 0.5: only compact when the majority of a segment is dead. At T = 0.5: `SA_A = 2.0` - the best Option A can achieve, using 2x the disk of your live data size.
...
- `L -> inf` (objects live forever): k = T/(1-T), so WA = 1/(1-T)

In practice WA sits somewhere between these depending on how long your objects live relative to your compaction rate. The benchmark measures actual WA directly as total bytes written / total user bytes written, which captures all of this without needing to know L.
...
```
minimize over T:    α x WA  +  β x SA
```

For Option A with no headroom (f = 0), SA = 1/T and WA = 1 + k(T, L). Using the L = 1 approximation as a baseline, WA ≈ 1 + T. The optimizer finds T* = sqrt(β/α), capped at 0.5. For any weighting that cares at all about space, T = 0.5 is optimal.
...
```
Live bytes  = 0.5 x 512 MB  =  256 MB
SA  =  512 / 256  =  2.0
```
...
```
Data capacity  = (1 - 0.33) x 512 MB  =  342 MB
Live bytes     = 0.5 x 342 MB          =  171 MB
SA  =  512 / 171  =  3.0
```

SA jumped from 2.0 to 3.0 purely because of headroom - 170 MB per segment is permanently reserved for compaction writes.

Headroom must fit all survivors from one compaction. A cold segment has `T x (1-f) x S` live bytes that need to land in headroom `f x S`:
...
At T = 0.5: SA_B = 3.0. At T = 0.3: SA_B = 4.3. Now both WA and SA for both options are functions of T alone.
...
```
minimize over T:    α x (1 + T)  +  β x (1 + T) / T

subject to:         0 < T <= 0.5
```

Closed form: `T* = sqrt(β / α), capped at 0.5` and `f* = T* / (1 + T*)`

| Priority | α | β | T* | f* | WA | SA |
|---|---|---|---|---|---|---|
| Only WA | 1.0 | 0.0 | -> 0 | -> 0 | -> 1.0 | -> inf |
| WA-heavy | 0.9 | 0.1 | 0.33 | 0.25 | 1.33 | 4.00 |
| Balanced | 0.5 | 0.5 | 0.50 | 0.33 | 1.50 | 3.00 |
| SA-heavy | 0.1 | 0.9 | 0.50 | 0.33 | 1.50 | 3.00 |
| Only SA | 0.0 | 1.0 | -> inf | -> 1 | -> inf | -> 1.0 |

The pure extremes are degenerate - one wastes all disk, the other rewrites all data constantly. For anything in between, T = 0.5 and f = 0.33 is the answer.
...
B always has worse SA. The only reason to choose B is if compaction into the active segment measurably slows fresh writes.

We measure the interference as ρ = throughput_idle / throughput_during_compaction. When ρ > 1, WA stops being a background cost and starts affecting user-visible write latency. Option B eliminates this by routing compaction into `active - 1`.

We solve each option for its optimal T independently and compare their minimum total costs:

```
Cost_A(ρ) = minimized over T:  α x (1+T) x ρ  +  β/T
Cost_B     = minimized over T:  α x (1+T)      +  β x (1+T)/T
```
...
|---|---|---|
| Write-latency-sensitive (live traffic, real-time ingestion) | Potentially high | Prefer B |
| Batch / archive (backups, cold storage) | Low | Prefer A - better SA, ρ doesn't matter |
| High churn (frequent deletes, overwrites) | Elevated | Lean toward B |
| Low churn (mostly writes, rare deletes) | Low | A is fine |

T should track your delete rate. High delete rate: T = 0.4-0.5. Bursty or infrequent deletes: T = 0.2-0.3. T can be static or made adaptive per bucket. We start with T = 0.30 and revisit once we have production data.
...
Somewhere in the middle of writing this - the steps, the optimization problem, the trade-offs - I sat back and realized I had written down things I had already seen. Every iteration showed up somewhere in one of these systems. Frustrating in a funny way.

This is probably why my approach has always been product-first. Systems design is important but it has a ceiling - the ideas are largely settled, the parameters are workload-dependent, and the real question was never "how do you manage storage" but "who are you managing it for." I had fun with the math. But the world does not need another well-formulated space amplification derivation. It needs someone to know when to stop deriving and start asking whether the person on the other end of the system actually cares.
